...
import org.seekloud.medusa.core.RoomActor
import org.seekloud.medusa.core.RoomActor.DeadInfo
import org.seekloud.medusa.snake.Protocol.{fSpeed, square}
import org.seekloud.medusa.core.RoomActor.Command
...
/**
  ...
  * Time: 9:55 PM
  */
class GridOnServer(override val boundary: Point, roomActor:ActorRef[Command]) extends Grid {
  ...
  def updateASnake(snake: SnakeInfo, actMap: Map[String, Int]): Either[String, SnakeInfo] = {
    ...
    while(step > 0) {
      ...
      if(distance >= step) { //尾巴在移动到下一个节点前就需要停止。
        ...
        removePoint ++= snake.tail.to(newTail).map{e=>(e,Body(snake.id, snake.color))}
        step = -1
        ...
        removePoint ++= old.to(newTail).map{e=>(e,Body(snake.id, snake.color))}
